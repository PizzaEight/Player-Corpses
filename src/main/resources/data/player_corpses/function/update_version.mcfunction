execute unless entity @s[predicate=player_corpses:version_number] run tellraw @s {"bold":true,"color":"dark_green","text":"Player Corpses successfully updated to version 2.9"}
scoreboard players set @s player_corpses.version 29
...
execute as @a unless score Config load matches 1 unless score Config remove_config matches 1 run function player_corpses:config/show
