summon minecraft:mannequin ~ ~ ~ {Team:"player_corpses.corpse_friend",Silent:1b,pose:sleeping,Tags:["player_corpses.26","player_corpses.corpse","fresh","player_corpses.loaded"],CustomNameVisible:1b,CustomName:"",description:{"color":aqua,"text":"Sneak here to get your items back"}}
...
execute unless items entity @s player.crafting.* * unless items entity @p player.cursor * unless data entity @s equipment unless data entity @s Inventory[] run tag @n[type=minecraft:mannequin,tag=player_corpses.corpse,tag=fresh] add player_corpses.poor

execute as @s at @s if entity @n[type=minecraft:mannequin,tag=player_corpses.corpse,tag=fresh,tag=!player_corpses.poor] run function player_corpses:items_add
execute as @s at @s if entity @n[type=minecraft:mannequin,tag=player_corpses.corpse,tag=fresh,tag=player_corpses.poor] run function player_corpses:items_add_poor
...
execute unless block ~ ~ ~ #air run tp @n[type=minecraft:mannequin,tag=player_corpses.corpse,tag=fresh] ~ ~0.5 ~
